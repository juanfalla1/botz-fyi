...
async function handle(req: Request, input: { tenant_id: string | null; user_id: string | null }) {
  ...
  if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
    return NextResponse.json({ ok: false, error: "MISSING_GOOGLE_ENV" }, { status: 500 });
  }
  ...
  // If user_id is provided, only platform admins can act on behalf of someone else.
  if (input.user_id && input.user_id !== user.id) {
    const guard = await assertTenantAccess({ req, requestedTenantId: input.tenant_id });
    if (!guard.ok || !guard.isPlatformAdmin) {
  ...
  const effectiveTenantId = guard.tenantId;
  const effectiveUserId = input.user_id || user.id;

  if (!isUuid(effectiveTenantId) || !isUuid(effectiveUserId)) {
  ...
  if (integErr) {
    return NextResponse.json({ ok: false, error: "DB_READ_FAILED", details: integErr.message }, { status: 500 });
  }
  if (!integ) {
    return NextResponse.json({ ok: false, error: "NO_INTEGRATION_FOUND" }, { status: 404 });
  }
  if (integ.status !== "connected") {
    return NextResponse.json({ ok: false, error: "NOT_CONNECTED", status: integ.status }, { status: 400 });
  }
  ...
  const mustRefresh =
    !access_token || (expires_at && Number.isFinite(expires_at) && expires_at <= now + 60);
  ...
  if (mustRefresh) {
    ...
    if (!tokenRes.ok) {
      return NextResponse.json({ ok: false, error: "TOKEN_REFRESH_FAILED", details: tokenJson }, { status: 401 });
    }
    ...
    if (upErr) {
      return NextResponse.json({ ok: false, error: "DB_UPDATE_FAILED", details: upErr.message }, { status: 500 });
    }
  }
  ...
  if (!gmailRes.ok) {
    return NextResponse.json({ ok: false, error: "GMAIL_API_FAILED", details: gmailJson }, { status: 502 });
  }
...
export async function GET(req: Request) {
  try {
    ...
    const tenant_id = String(url.searchParams.get("tenant_id") || "").trim() || null;
    return await handle(req, { tenant_id, user_id: null });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: "UNEXPECTED_ERROR", details: e?.message || String(e) }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const tenant_id = String(body?.tenant_id || "").trim() || null;
    const user_id = String(body?.user_id || "").trim() || null;
    return await handle(req, { tenant_id, user_id });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: "UNEXPECTED_ERROR", details: e?.message || String(e) }, { status: 500 });
  }
}
