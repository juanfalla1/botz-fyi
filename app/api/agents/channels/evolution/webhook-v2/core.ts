...
function isOutOfCatalogDomainQuery(text: string): boolean {
  const t = normalizeText(text || "");
  if (!t) return false;
  const asksTechnicalDocs = /(ficha|ficha tecnica|datasheet|hoja tecnica|especificaciones|manual|brochure|catalogo tecnico)/.test(t);
  const hasLikelyCatalogModel = /\b(ax\d|px\d|exr?\d|exp\d|r31|rc31|r71|mb\d|valor|defender|ranger|scout|adventurer|explorer|pioneer)\b/.test(t);
  if (asksTechnicalDocs && hasLikelyCatalogModel) return false;
  ...
  try {
    ...
    if (STRICT_REBUILD_MODE) {
      ...
      const recentAssistantMenuHint = historyMessages
        .slice(-2)
        .some((m) => m?.role === "assistant" && /que deseas ahora\?|1\)\s*cotizaci[oó]n[\s\S]{0,80}2\)\s*ficha\s+t[eé]cnica/i.test(String(m?.content || "")));
      const isActionMenuNumericReply = /^\s*[12]\s*$/.test(textNorm);
      const forceActionMenuRouting = isActionMenuNumericReply && (awaiting === "strict_choose_action" || wasModelActionMenu || recentAssistantMenuHint);
      const isDirectSheetOptionInAction = /^\s*2\s*$/.test(textNorm) && forceActionMenuRouting;
      const isDirectQuoteOptionInAction = /^\s*1\s*$/.test(textNorm) && forceActionMenuRouting;
      const wantsSheet = isTechnicalSheetIntent(text) || isDirectSheetOptionInAction;
      const wantsQuote = asksQuoteIntent(text) || isPriceIntent(text) || isDirectQuoteOptionInAction;
      if (forceActionMenuRouting) {
        strictMemory.awaiting_action = "strict_choose_action";
        strictMemory.last_menu_type = "";
      }
      ...
      if (!String(strictReply || "").trim() && !awaiting && isAffirmativeShortIntent(text) && hasContextForGuidance && !explicitModel && !wantsQuote && !wantsSheet) {
        ...
        strictMemory.last_selected_product_snapshot = {
          id: String(selectedProduct?.id || "").trim() || null,
          name: selectedName || null,
          category: String((selectedProduct as any)?.category || "").trim() || null,
          base_price_usd: Number((selectedProduct as any)?.base_price_usd || 0) || null,
          source_payload: ((selectedProduct as any)?.source_payload && typeof (selectedProduct as any).source_payload === "object")
            ? (selectedProduct as any).source_payload
            : null,
          datasheet_url: String((selectedProduct as any)?.datasheet_url || "").trim() || null,
        };
      ...
      if (
        ...
        strictMemory.strict_quote_data_missing_attempts = 0;
        {
          ...
          const selectedSnapshot = previousMemory?.last_selected_product_snapshot && typeof previousMemory.last_selected_product_snapshot === "object"
            ? previousMemory.last_selected_product_snapshot
