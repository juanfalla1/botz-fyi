...
export async function GET(req: Request) {
  try {
    ...
    const ranking = groupByDimension(facts, "customer");
    const enriched = ranking.slice(0, 200).map((r) => {
      const cf = facts.filter((f) => f.customer === r.label);
      return {
        ...r,
        frequency: cf.length,
        topProducts: groupByDimension(cf, "product").slice(0, 3),
        topCategories: groupByDimension(cf, "category").slice(0, 3),
        topCities: groupByDimension(cf, "city").slice(0, 2),
      };
    });
    return NextResponse.json({ ok: true, ranking: enriched });
  } catch (error: any) {
    return NextResponse.json({ ok: false, error: error?.message || "Error customers" }, { status: 500 });
  }
}
