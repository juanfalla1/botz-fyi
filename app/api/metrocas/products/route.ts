...
export async function GET(req: Request) {
  try {
    ...
    const ranking = groupByDimension(facts, "product");
    const stars = ranking.slice(0, 20);
    const lowRotation = [...ranking].sort((a, b) => a.quantity - b.quantity).slice(0, 20);
    return NextResponse.json({ ok: true, ranking, stars, lowRotation });
  } catch (error: any) {
    return NextResponse.json({ ok: false, error: error?.message || "Error products" }, { status: 500 });
  }
}
