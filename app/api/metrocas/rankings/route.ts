...
export async function GET(req: Request) {
  ...
  let query = svc.from("metrocas_rankings").select("*").eq("tenant_id", access.tenantId).eq("dataset_id", datasetId);
  if (type) query = query.eq("ranking_type", type);
  const { data, error } = await query.order("ranking_order", { ascending: true });
